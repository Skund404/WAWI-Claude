...
def fix_storage_table():
    ...
    try:
        ...
        column_names = [col[1] for col in columns]
        logger.info(f'Current storage table columns: {column_names}')
        expected_columns = ['id', 'name', 'location', 'capacity',
            'current_occupancy', 'type', 'description', 'status']
...
def add_sample_data():
    ...
    try:
        ...
        if count == 0:
            ...
            sample_data = [('Main Warehouse', 'Building A', 1000.0, 650.0,
                'Warehouse', 'Main storage facility', 'active'), (
                'Shelf A1', 'Main Warehouse - Section A', 50.0, 30.0,
                'Shelf', 'Small parts storage', 'active'), ('Shelf B2',
                'Main Warehouse - Section B', 75.0, 45.0, 'Shelf',
                'Medium parts storage', 'active'), ('Cold Storage',
                'Building B', 500.0, 200.0, 'Refrigerated',
                'Temperature controlled storage', 'active'), (
                'Archive Room', 'Building C', 300.0, 280.0, 'Archive',
                'Document and sample storage', 'active')]
            cursor.execute('PRAGMA table_info(storage);')
            columns = cursor.fetchall()
            column_names = [col[1] for col in columns]
            if 'id' in column_names:
                column_names.remove('id')
            needed_columns = ['name', 'location', 'capacity',
                'current_occupancy', 'type', 'description', 'status']
            missing_columns = [col for col in needed_columns if col not in
                column_names]
            if missing_columns:
                logger.error(
