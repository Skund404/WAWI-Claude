...
def check_module_imports():
    ...
    """
    modules_to_check = ['config', 'database', 'gui', 'services', 'utils',
...
def check_database_config():
    ...
    """
    root = get_project_root()
    config_files = [root / 'config' / 'settings.py', root / 'config' /
        'config.py', root / 'config.py', root / 'settings.py']
        ...
        if config_file.exists():
            ...
            try:
                with open(config_file, 'r') as f:
                    content = f.read()
                    if any(term in content for term in ['database',
                        'db_path', 'sqlite']):
                        logger.info(
                            f'Config file {config_file} contains database settings'
                            )
                        lines = content.split('\n')
                        db_lines = [line for line in lines if any(term in
                            line.lower() for term in ['database', 'db_path',
                            'sqlite'])]
