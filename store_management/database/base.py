...
# Create metadata and registry - these MUST come before any imports that might use them
metadata = MetaData()
...
class Base(DeclarativeBase):
    ...
    # Add class methods for model queries
    @classmethod
    def list_models(cls):
        """Return a list of all registered model names"""
        return list(ModelRegistry._registry.keys())
...
# For backwards compatibility - some models might still use BaseModel
BaseModel = Base
...
try:
    ...
    # Register the Base class for lazy imports
    register_lazy_import('Base', 'database.models.base', 'Base')
    register_lazy_import('metadata', 'database.models.base', 'metadata')
    register_lazy_import('mapper_registry', 'database.models.base', 'mapper_registry')
except ImportError as e:
    logger.error(f"Failed to import circular_import_resolver: {e}")


    # Provide fallback implementations to avoid breaking the application
    def register_lazy_import(*args, **kwargs):
        logger.warning("Using fallback register_lazy_import")
        pass


    def resolve_lazy_import(*args, **kwargs):
        logger.warning("Using fallback resolve_lazy_import")
        return None
...
class CostingMixin:
    ...
    def update_cost(
        ...
        try:
            ...
            logger.warning("Failed to import QualityGrade enum for cost update")
...
class TrackingMixin:
    ...
    # Inventory and status tracking - use string names to avoid circular imports
        ...
        try:
            ...
            if session and hasattr(self, 'id'):
                try:
                    ...
                    logger.debug("StatusHistory model not available, skipping history recording")
        except ImportError as e:
            logger.error(f"Failed to update inventory status: {e}")
            raise ValueError(f"Failed to update inventory status: {e}")
...
class ComplianceMixin:
    ...
    def get_compliance_history(self, session: Session) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("ComplianceHistory model not available, returning empty history")
...
class AuditMixin:
    ...
    def soft_delete(self, user_id: Optional[str] = None) -> None:
        ...
        logger.info(f"Soft deleted {self.__class__.__name__} with ID {getattr(self, 'id', 'unknown')}")
    ...
    def restore(self, user_id: Optional[str] = None) -> None:
        ...
        logger.info(f"Restored {self.__class__.__name__} with ID {getattr(self, 'id', 'unknown')}")
        ...
        try:
            ...
            if module_path:
                try:
                    ...
                    logger.warning(f"Failed to register lazy import for {model_name}: {e}")
...
class EnhancedMetaclass(DeclarativeMeta):
    ...
    def __new__(mcs, name, bases, attrs, **kwargs):
        ...
        # Inject to_dict method if not present
        ...
        try:
            ...
            logger.error(f"Error creating class {name}: {e}", exc_info=True)
            raise
    ...
    def _generate_to_dict():
        ...
        def to_dict(
            ...
            try:
                ...
                logger.warning(f"Error getting columns in to_dict: {e}")
                # Fallback to using all attributes
            ...
            if include_relationships and _current_depth < max_depth:
                try:
                    ...
                    logger.warning(f"Error processing relationships in to_dict: {e}")
        ...
        def update_from_dict(
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"Error getting columns in update_from_dict: {e}")
                    # Fallback to using all attributes
                    updatable_columns = [
                ...
                if modified_fields:
                    ...
                    logger.debug(
                        f"Updated {len(modified_fields)} fields on {self.__class__.__name__}: {fields_str}")
                ...
                logger.error(f"Error in update_from_dict: {e}")
                raise
...
def create_base_model(
    ...
    try:
        ...
        logger.error(f"Error creating base model: {e}", exc_info=True)
        raise
...
def apply_mixins(base_class: Type, *mixins) -> Type:
    ...
    try:
        ...
        logger.error(f"Error applying mixins: {e}", exc_info=True)
        raise
...
# Add model event listeners for tracking changes
...
def after_insert(mapper, connection, target):
    ...
    try:
        if hasattr(target, 'id'):
            ...
            logger.info(f"Created {target.__class__.__name__} instance")
    except Exception as e:
...
def after_update(mapper, connection, target):
    ...
    try:
        if hasattr(target, 'id'):
            ...
            logger.info(f"Updated {target.__class__.__name__} instance")
    except Exception as e:
...
def after_delete(mapper, connection, target):
    ...
    try:
        if hasattr(target, 'id'):
            ...
            logger.info(f"Deleted {target.__class__.__name__} instance")
    except Exception as e:
...
def initialize_all_model_relationships():
    ...
    This can be called after all models are loaded to ensure all relationships are set up.
    """
    try:
        # Try to import and use the relationship initializer from init_relationships
        from .init_relationships import initialize_database_relationships
        ...
        logger.warning("Could not import relationship initializer. Skipping relationship initialization.")
    except Exception as e:
        logger.error(f"Error initializing model relationships: {e}", exc_info=True)


# Define a ModelFactory class for the export list
class ModelFactory:
    """
    Factory for creating model instances with default values.
    """
