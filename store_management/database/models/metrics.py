...
class MetricSnapshot(BaseModel):
    ...
    def __repr__(self):
        return f"<MetricSnapshot(type={self.metric_type}, time={self.timestamp})>"
...
class MaterialUsageLog(BaseModel):
    ...
    def calculate_efficiency(self) -> float:
        if self.quantity_used == 0:
            return 0.0
        return (self.quantity_used - self.waste_amount) / self.quantity_used
...
from database.models.metrics import MetricSnapshot, MaterialUsageLog, MetricType, TimeFrame
...
class MetricsRepository:
    ...
    def get_latest_metrics(self, metric_type: MetricType) -> Optional[MetricSnapshot]:
        ...
        return self.session.query(MetricSnapshot) \
    ...
    def get_material_efficiency_stats(
        ...
        result = query.first()
        return {
            'average_efficiency': float(result.avg_efficiency) if result.avg_efficiency else 0.0,
            'total_waste': float(result.total_waste) if result.total_waste else 0.0,
            'total_used': float(result.total_used) if result.total_used else 0.0
        }

    def cleanup_old_metrics(self, days_to_keep: int = 365) -> int:
        """Clean up old metric snapshots"""
        cutoff_date = datetime.utcnow() - timedelta(days=days_to_keep)
        deleted = self.session.query(MetricSnapshot) \
...
class DashboardService(IBaseService):
    ...
    def _get_inventory_metrics(self) -> Dict:
        ...
        try:
            low_stock_parts = self.inventory_service.get_low_stock_parts(include_out_of_stock=True)
            low_stock_leather = self.inventory_service.get_low_stock_leather(include_out_of_stock=True)

            return {
                'leather_stock_level': self._calculate_stock_percentage(low_stock_leather),
                'hardware_stock_level': self._calculate_stock_percentage(low_stock_parts),
                'low_stock_count': len(low_stock_parts) + len(low_stock_leather),
    ...
    def _get_project_metrics(self) -> Dict:
        ...
        try:
            ...
            completed_projects = self.project_service.get_completed_projects(
            ...
            return {
                ...
                'completion_rate': self._calculate_completion_rate(active_projects),
                'delayed_projects': self._count_delayed_projects(active_projects)
            }
        except Exception as e:
    ...
    def _get_material_usage(self) -> Dict:
        ...
        try:
            ...
            usage_stats = self.metrics_repository.get_material_efficiency_stats(days=30)
            usage_history = self.metrics_repository.get_material_usage_history(
    ...
    def _get_supplier_metrics(self) -> List[Dict]:
        ...
        try:
            ...
            for supplier in suppliers:
                performance = self.supplier_service.get_supplier_performance(supplier.id)
                metrics.append({
    ...
    def _calculate_stock_percentage(self, items: List) -> float:
        if not items:
            return 100.0
        optimal_items = sum(1 for item in items if not item.needs_reorder())
    ...
    def _get_default_inventory_metrics(self) -> Dict:
        return {
            'leather_stock_level': 0,
            'hardware_stock_level': 0,
            'low_stock_count': 0,
            'pending_orders': 0
        }
