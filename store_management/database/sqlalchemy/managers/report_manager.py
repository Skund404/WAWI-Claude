...
class ReportManager(BaseManager):
        ...
        query = self.session.query(Storage.id, Storage.bin, Product.name.
            label('product_name'), Product.unique_id.label('product_id'),
...
class ReportDialog(BaseDialog):
        ...
        ttk.Button(button_frame, text='Close', command=self.destroy).pack(side
            =tk.LEFT, padx=5)
        ...
        if report_type == 'inventory':
            ...
            self._add_filter('product_name', 'Product Name:', widget_type=
                'entry')
            ...
            self._add_filter('product_name', 'Product Name:', widget_type=
                'entry')
            self._add_filter('recipe_name', 'Project Name:', widget_type=
                'entry')
        elif report_type == 'low_stock':
            self._add_filter('min_threshold', 'Min Threshold:', widget_type
                ='entry')
