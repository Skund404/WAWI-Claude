...
"""
Manager for handling incoming goods and order operations.
"""
...
class IncomingGoodsManager(BaseManager[Order]):
    """
    Specialized manager for handling incoming goods and order operations.
    """
    ...
    def create_order(self, data: Dict[str, Any]) -> Order:
        """
        Create a new order with validation and tracking.

        Args:
            data (Dict[str, Any]): Order data including order number, supplier, etc.
    ...
    def get_order_by_id(self, order_id: int) -> Optional[Order]:
        ...
        try:
            ...
            logger.error(f'Failed to retrieve order by ID {order_id}: {str(e)}')
            raise DatabaseError(f'Failed to retrieve order by ID {order_id}: {str(e)}') from e
    ...
    def get_order_by_number(self, order_number: str) -> Optional[Order]:
        ...
        try:
            ...
            logger.error(f'Failed to retrieve order by number {order_number}: {str(e)}')
            raise DatabaseError(f'Failed to retrieve order by number {order_number}: {str(e)}') from e
    ...
    def update_order(self, order_id: int, data: Dict[str, Any]) -> bool:
        ...
        Args:
            ...
            data (Dict[str, Any]): Updated order data
        ...
        try:
            ...
            raise DatabaseError(f'Failed to update order {order_id}: {str(e)}') from e
    ...
    def delete_order(self, order_id: int) -> bool:
        ...
        Returns:
            bool: True if deletion successful, False if order not found
        """
        try:
            order = self.get_order_by_id(order_id)
            if order:
            ...
            raise DatabaseError(f'Failed to delete order {order_id}: {str(e)}') from e
    ...
    def add_order_detail(self, order_id: int, data: Dict[str, Any]) -> OrderDetail:
        ...
        Args:
            order_id (int): ID of the order to add detail to
            data (Dict[str, Any]): Order detail data

        Returns:
            OrderDetail: Created order detail instance
        """
        try:
            detail = OrderDetail(
                order_id=order_id,
            ...
            self.session.rollback()
            logger.error(f'Failed to add order detail for order {order_id}: {str(e)}')
            raise DatabaseError(f'Failed to add order detail for order {order_id}: {str(e)}') from e
    ...
    def get_order_details(self, order_id: int) -> List[OrderDetail]:
        ...
        Args:
            order_id (int): ID of the order to retrieve details for

        Returns:
            List[OrderDetail]: List of order details
        """
        try:
            return self.session.query(OrderDetail).filter_by(order_id=order_id).all()
        except Exception as e:
            logger.error(f'Failed to retrieve order details for order {order_id}: {str(e)}')
            raise DatabaseError(f'Failed to retrieve order details for order {order_id}: {str(e)}') from e
    ...
    def update_order_detail(self, detail_id: int, data: Dict[str, Any]) -> bool:
        ...
        Args:
            detail_id (int): ID of the order detail to update
            data (Dict[str, Any]): Updated order detail data
        ...
        try:
            ...
            self.session.rollback()
            logger.error(f'Failed to update order detail {detail_id}: {str(e)}')
            raise DatabaseError(f'Failed to update order detail {detail_id}: {str(e)}') from e
    ...
    def delete_order_detail(self, detail_id: int) -> bool:
        ...
        Args:
            detail_id (int): ID of the order detail to delete
        ...
        try:
            ...
            self.session.rollback()
            logger.error(f'Failed to delete order detail {detail_id}: {str(e)}')
            raise DatabaseError(f'Failed to delete order detail {detail_id}: {str(e)}') from e
