...
class OrderManager(BaseManager[Order]):
    ...
    def create_order(self, order_data: Dict[str, Any], items: List[Dict[str, Any]]) -> Order:
        """
        ...
        try:
            ...
            if missing_fields:
                raise DatabaseError(f"Missing required order fields: {', '.join(missing_fields)}")
    ...
    def update_order_status(self, order_id: int, status: str) -> Optional[Order]:
        ...
        if status not in valid_statuses:
            raise DatabaseError(f"Invalid status. Must be one of: {', '.join(valid_statuses)}")

        return self.update(order_id, {'status': status, 'modified_at': datetime.utcnow()})

    def add_order_items(self, order_id: int, items: List[Dict[str, Any]]) -> Order:
        """
