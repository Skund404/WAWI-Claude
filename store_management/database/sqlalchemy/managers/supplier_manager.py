...
class SupplierManager(BaseManager[Supplier]):
    ...
    def get_supplier_products(self, supplier_id: int) -> Dict[str, List]:
        ...
        with self.session_scope() as session:
            ...
            leather = session.query(Leather).filter(
                Leather.supplier_id == supplier_id
            ).all()
    ...
    def update_supplier_rating(self, supplier_id: int, rating: float,
                               notes: Optional[str] = None) -> Supplier:
        """
    ...
    def get_supplier_order_history(self, supplier_id: int,
    ...
    def get_top_suppliers(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        with self.session_scope() as session:
            ...
            suppliers = session.query(Supplier).options(
                joinedload(Supplier.orders)
            ).all()
            ...
            for supplier in suppliers:
                ...
                completed_orders = sum(1 for o in supplier.orders
                                       if o.status == 'completed')
                on_time_orders = sum(1 for o in supplier.orders
            ...
            # Sort by performance score and return top suppliers
            return sorted(supplier_metrics,
    ...
    def get_supplier_categories(self, supplier_id: int) -> List[str]:
        ...
        with self.session_scope() as session:
            ...
            part_categories = session.query(Part.category).filter(
                Part.supplier_id == supplier_id
            ).distinct().all()

            # Get unique leather types
            leather_types = session.query(Leather.type).filter(
                Leather.supplier_id == supplier_id
            ).distinct().all()

            # Combine and deduplicate
            categories = (
                    [cat[0] for cat in part_categories if cat[0]] +
                    [type_[0] for type_ in leather_types if type_[0]]
            )
    ...
    def search_suppliers(self, search_term: str) -> List[Supplier]:
        ...
        query = select(Supplier).filter(
            or_(
                ...
                Supplier.notes.ilike(f"%{search_term}%")
            )
        )
    ...
    def get_supplier_statistics(self) -> Dict[str, Any]:
        ...
        with self.session_scope() as session:
            ...
            return {
                ...
                'total_orders': total_orders,
                'orders_per_supplier': round(total_orders / total_suppliers, 2) if total_suppliers > 0 else 0
            }