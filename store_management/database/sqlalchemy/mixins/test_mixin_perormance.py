...
def large_dataset(performance_manager):
    ...
    def _populate(session):
        records = [PerformanceTestModel(name=f'Item {i}', category='test' if
                                        i % 5 == 0 else 'regular', status='active' if i % 3 == 0 else
                                        'inactive', value=i) for i in range(10000)]
...
class TestMixinPerformance:
    ...
    @inject(MaterialService)
        ...
        @inject(MaterialService)
            def test_advanced_search_performance(self, performance_manager,
                                             large_dataset):
        """
        Benchmark advanced search performance.
        """
        start_time = time.time()
        results = performance_manager.advanced_search({'category': {'op':
                                                                    '==', 'value': 'test'}, 'value': {'op': '>', 'value': 1000}})
        search_time = time.time() - start_time
        assert len(results) > 0
        ...
        """
        start_time = time.time()
        results = performance_manager.filter_by_multiple({'status':
        ...
        """
        start_time = time.time()
        results = performance_manager.get_paginated(page=2, page_size=100,
                                                    order_by='value', filters={'status': 'active'})
        pagination_time = time.time() - start_time
        assert len(results['items']) == 100
        assert pagination_time < 0.5, f'Pagination took {pagination_time} seconds, expected < 0.5'

        @inject(MaterialService)
            def test_complex_filter_performance(self, performance_manager,
                                            large_dataset):
        """
        Benchmark complex filtering performance.
        """
        start_time = time.time()
        results = performance_manager.filter_complex([{'field': 'status',
                                                       'op': '==', 'value': 'active'}, {'field': 'category', 'op':
                                                                                        '==', 'value': 'test'}, {'field': 'value', 'op': '>', 'value':
                                                                                                                 500}])
        ...
        @inject(MaterialService)
            def test_native_sqlalchemy_performance_comparison(self,
                                                          performance_manager, large_dataset):
        """
        Compare mixin performance with native SQLAlchemy query.
        """
        mixin_start_time = time.time()
        mixin_results = performance_manager.filter_complex([{'field':
                                                             'status', 'op': '==', 'value': 'active'}, {'field': 'category',
                                                                                                        'op': '==', 'value': 'test'}, {'field': 'value', 'op': '>',
                                                                                                                                       'value': 500}])
