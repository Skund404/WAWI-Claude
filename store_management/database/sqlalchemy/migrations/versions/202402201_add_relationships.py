...
def upgrade():
    with op.batch_alter_table("leather") as batch_op:
        ...
        batch_op.add_column(
            sa.Column("minimum_area", sa.Float(), nullable=True))
        ...
        with op.batch_alter_table("production_orders") as batch_op:
            ...
            batch_op.create_index("idx_production_dates", [
            ...
            with op.batch_alter_table("produced_items") as batch_op:
                ...
                batch_op.add_column(
                    sa.Column("quality_check_passed",
                              sa.Boolean(), nullable=True)
                )
                batch_op.create_foreign_key(
                    "fk_produced_recipe", "patterns", ["recipe_id"], ["id"], ondelete="RESTRICT"
                )
                batch_op.create_unique_constraint(
                    "uq_produced_serial", ["serial_number"])
                ...
                with op.batch_alter_table("patterns") as batch_op:
                    ...
                    op.execute(
                        "UPDATE patterns SET version = '1.0' WHERE version IS NULL")
                    batch_op.create_unique_constraint(
                        "uq_recipe_name_version", ["name", "version"])
                    batch_op.create_index("idx_recipe_category", ["category"])
                    with op.batch_alter_table("orders") as batch_op:
                        batch_op.add_column(
                            sa.Column("supplier_id", sa.Integer(), nullable=True))
                        batch_op.add_column(
                            sa.Column("expected_delivery",
                        ...
                        batch_op.create_index(
                            "idx_order_supplier", ["supplier_id"])
                        batch_op.create_index("idx_order_dates", [
