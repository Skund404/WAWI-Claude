...
def upgrade():
    with op.batch_alter_table('leather') as batch_op:
        batch_op.add_column(sa.Column('supplier_id', sa.Integer(), nullable
            =True))
        batch_op.create_foreign_key('fk_leather_supplier', 'suppliers', [
        ...
        batch_op.add_column(sa.Column('minimum_area', sa.Float(), nullable=
            True))
        op.execute(
            "UPDATE leather SET status = 'in_stock' WHERE status IS NULL")
        op.execute(
            'UPDATE leather SET minimum_area = 0 WHERE minimum_area IS NULL')
    ...
    with op.batch_alter_table('production_orders') as batch_op:
        ...
        batch_op.add_column(sa.Column('status', sa.String(20), nullable=True))
        batch_op.add_column(sa.Column('start_date', sa.DateTime(), nullable
            =True))
        batch_op.add_column(sa.Column('completion_date', sa.DateTime(),
            nullable=True))
        batch_op.create_foreign_key('fk_production_recipe', 'patterns', [
            'recipe_id'], ['id'], ondelete='RESTRICT')
        op.execute(
            "UPDATE production_orders SET status = 'planned' WHERE status IS NULL"
            )
        batch_op.alter_column('recipe_id', nullable=False)
        batch_op.alter_column('status', nullable=False)
        batch_op.create_index('idx_production_status', ['status'])
        batch_op.create_index('idx_production_dates', ['start_date',
            'completion_date'])
    with op.batch_alter_table('produced_items') as batch_op:
        batch_op.add_column(sa.Column('recipe_id', sa.Integer(), nullable=True)
            )
        batch_op.add_column(sa.Column('serial_number', sa.String(50),
            nullable=True))
        batch_op.add_column(sa.Column('quality_check_passed', sa.Boolean(),
            nullable=True))
        batch_op.create_foreign_key('fk_produced_recipe', 'patterns', [
            'recipe_id'], ['id'], ondelete='RESTRICT')
        batch_op.create_unique_constraint('uq_produced_serial', [
            'serial_number'])
    ...
    with op.batch_alter_table('patterns') as batch_op:
        ...
        op.execute(
            'UPDATE patterns SET is_active = TRUE WHERE is_active IS NULL')
        op.execute("UPDATE patterns SET version = '1.0' WHERE version IS NULL")
        batch_op.create_unique_constraint('uq_recipe_name_version', ['name',
            'version'])
    ...
    with op.batch_alter_table('orders') as batch_op:
        ...
        batch_op.add_column(sa.Column('total_amount', sa.Numeric(10, 2),
            nullable=True))
        batch_op.create_foreign_key('fk_order_supplier', 'suppliers', [
            'supplier_id'], ['id'], ondelete='RESTRICT')
        batch_op.create_index('idx_order_supplier', ['supplier_id'])
        batch_op.create_index('idx_order_dates', ['order_date',
            'expected_delivery'])
