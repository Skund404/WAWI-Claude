...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('project_type', sa.Enum(
            'WALLET', 'BAG', 'BELT', 'JACKET', 'ACCESSORY', 'SHOE', 'GLOVES', 'CUSTOM',
            name='projecttype'), nullable=False),
        sa.Column('skill_level', sa.Enum(
            'BEGINNER', 'INTERMEDIATE', 'ADVANCED', 'MASTER',
            name='skilllevel'), nullable=False),
        sa.Column('production_status', sa.Enum(
            'DESIGN', 'PROTOTYPE', 'IN_PRODUCTION', 'COMPLETED', 'ON_HOLD',
            name='productionstatus'), nullable=True),
        ...
        sa.Column('primary_leather_type', sa.String(length=100), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('component_type', sa.Enum(
            'MAIN_BODY', 'STRAP', 'POCKET', 'LINING',
            'HARDWARE_ATTACHMENT', 'CLOSURE', 'DECORATIVE', 'OTHER',
            name='componenttype'), nullable=False),
        sa.Column('length', sa.Float(), nullable=True),
        sa.Column('width', sa.Float(), nullable=True),
        sa.Column('material_type', sa.String(length=100), nullable=True),
        sa.Column('material_thickness', sa.Float(), nullable=True),
        sa.Column('cutting_pattern_reference', sa.String(length=50), nullable=True),
        sa.Column('estimated_cutting_time', sa.Float(), nullable=True),
        sa.Column('quality_score', sa.Float(), nullable=True),
        sa.Column('wastage_percentage', sa.Float(), nullable=True),
        sa.Column('cutting_instructions', sa.String(length=500), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('hardware_type', sa.Enum(
            'BUCKLE', 'RIVET', 'SNAP', 'ZIPPER', 'D_RING', 'LOCK',
            'CLASP', 'BUTTON', 'HOOK', 'CHAIN', 'BRACKET', 'SLIDER',
            'CONNECTOR', 'OTHER',
            name='hardwaretype'), nullable=False),
        sa.Column('material', sa.Enum(
            'BRASS', 'STAINLESS_STEEL', 'NICKEL', 'BRONZE', 'ALUMINUM',
            'GOLD_PLATED', 'SILVER', 'COPPER', 'TITANIUM', 'PLASTIC',
            name='hardwarematerial'), nullable=False),
        sa.Column('width', sa.Float(), nullable=True),
        sa.Column('length', sa.Float(), nullable=True),
    ...
    # Create many-to-many association table for hardware and project components
    op.create_table(
...
def downgrade():
    # Provide a way to roll back the migration if needed
    op.drop_table('project_component_hardware')
    op.drop_table('hardware')
    op.drop_table('project_components')
    op.drop_table('projects')

    # Recreate original tables (you'd need to define the original schema)
    # This is a placeholder - you'd need to provide the exact original table definitions