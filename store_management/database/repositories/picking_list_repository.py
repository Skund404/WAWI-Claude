...
class PickingListRepository(BaseRepository[PickingList]):
    ...
    def get_by_sales_id(self, sales_id: int) -> Optional[PickingList]:
        ...
        """
        try:
            stmt = select(PickingList).filter(
                PickingList.sales_id == sales_id
            )

            picking_list = self.session.execute(stmt).scalar_one_or_none()
            if picking_list:
    ...
    def get_by_status(self, status: PickingListStatus) -> List[PickingList]:
        ...
        try:
            ...
            picking_lists = result.scalars().all()
    ...
    def mark_item_as_picked(self, item_id: int, quantity: Optional[int] = None) -> PickingListItem:
        ...
        """
        try:
            # Get the item - this was incorrectly written in the original code
            stmt = select(PickingListItem).filter(PickingListItem.id == item_id)
    ...
    def reset_item_picking_status(self, item_id: int) -> PickingListItem:
        ...
        """
        try:
            # Get the item - this had the same issue as the previous method
            stmt = select(PickingListItem).filter(PickingListItem.id == item_id)
            item = self.session.execute(stmt).scalar_one_or_none()
