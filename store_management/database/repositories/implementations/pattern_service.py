...
from models.pattern import Pattern, SkillLevel
...
class PatternRepository(IPatternRepository, IBaseRepository):
    ...
    def get_by_skill_level(self, skill_level: SkillLevel) -> List[Pattern]:
        ...
        """
        try:
            return self._session.query(Pattern).filter(Pattern.skill_level == skill_level).all()
        except Exception as e:
    ...
    def get_with_components(self, pattern_id: int) -> Pattern:
        ...
        """
        try:
            pattern = self._session.query(Pattern).filter(Pattern.id == pattern_id).options(joinedload(Pattern.components)).first()
            if not pattern:
                raise ApplicationError(f'Pattern with ID {pattern_id} not found')
            return pattern
        except Exception as e:
