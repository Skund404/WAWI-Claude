...
try:
    pass
pattern = self._session.query(Pattern).options(joinedload(
Pattern.components)).filter(Pattern.id == pattern_id).first()
if not pattern:
    pass
...
results = query.options(joinedload(Pattern.components)).limit(
...
f'patterns_skill_{pattern.skill_level}', 'all_patterns'])
return pattern
