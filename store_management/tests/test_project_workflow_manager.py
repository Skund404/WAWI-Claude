...
class TestProjectWorkflowManager(unittest.TestCase):
    ...
    @inject(MaterialService)
        ...
        @inject(MaterialService)
            ...
            project_id = self.workflow_manager.create_project(
                name='Test Project', project_type=ProjectType.BAG, skill_level=SkillLevel.INTERMEDIATE, description='Test description')
            ...
            self.assertEqual(project['skill_level'],
                             SkillLevel.INTERMEDIATE.name)
            self.assertEqual(project['description'], 'Test description')
            self.assertEqual(project['status'], ProjectStatus.CONCEPT.name)

            @inject(MaterialService)
                def test_update_project_details(self):
                project_id = self.workflow_manager.create_project(name='Original Project', project_type=ProjectType.WALLET,
                ...
                @inject(MaterialService)
                    ...
                    project_id = self.workflow_manager.create_project(name='Status Test Project', project_type=ProjectType.BELT,
                    ...
                    project = self.workflow_manager.projects[project_id]
                    self.assertEqual(
                        project['status'], ProjectStatus.DESIGN.name)
                    self.assertEqual(len(project['history']), 1)

                    @inject(MaterialService)
                        def test_add_project_material(self):
                        project_id = self.workflow_manager.create_project(name='Material Test Project', project_type=ProjectType.ACCESSORY,
                        ...
                        @inject(MaterialService)
                            ...
                            project_id = self.workflow_manager.create_project(name='Usage Test Project', project_type=ProjectType.GARMENT,
                            ...
                            @inject(MaterialService)
                                ...
                                project_id = self.workflow_manager.create_project(name='Task Test Project', project_type=ProjectType.CUSTOM,
                                                                                  skill_level=SkillLevel.ADVANCED)
                                task_id = self.workflow_manager.add_project_task(
                                    project_id, name='Test Task', description='Task description', estimated_duration=timedelta(hours=2), priority=2)
                                project = self.workflow_manager.projects[project_id]
                                self.assertIn(
                                    task_id, [task['id'] for task in project['tasks']])
                                task = next(
                                    task for task in project['tasks'] if task['id'] == task_id)
                                ...
                                @inject(MaterialService)
                                    ...
                                    project_id = self.workflow_manager.create_project(name='Task Status Test Project', project_type=ProjectType.BAG,
                                                                                      skill_level=SkillLevel.BEGINNER)
                                    task_id = self.workflow_manager.add_project_task(
                                        project_id, name='Status Test Task')
                                    ...
                                    self.assertEqual(
                                        task['assigned_to'], 'John Doe')

                                    @inject(MaterialService)
                                        def test_generate_project_summary(self):
                                        project_id = self.workflow_manager.create_project(name='Summary Test Project', project_type=ProjectType.WALLET,
                                                                                          skill_level=SkillLevel.INTERMEDIATE, description='Test summary generation')
                                        self.workflow_manager.add_project_material(project_id,
                                                                                   'LEATHER-002', expected_quantity=3.0)
                                        self.workflow_manager.add_project_task(
                                            project_id, name='Summary Task')
                                        summary = self.workflow_manager.generate_project_summary(
                                            project_id)
                                        self.assertEqual(
                                            summary['name'], 'Summary Test Project')
                                        self.assertEqual(
                                            summary['type'], ProjectType.WALLET.name)
                                        self.assertEqual(
                                            summary['skill_level'], SkillLevel.INTERMEDIATE.name)
                                        self.assertEqual(
                                            summary['description'], 'Test summary generation')
                                        self.assertIn(
                                            'LEATHER-002', summary['materials'])
                                        self.assertEqual(
                                            len(summary['tasks']), 1)
