...
class StorageView(ttk.Frame):
        ...
        ttk.Button(toolbar, text='ADD', command=self.show_add_dialog).pack(side
            =tk.LEFT, padx=2)
        ...
        ttk.Entry(main_frame, textvariable=bin_var).grid(row=2, column=1,
            sticky='ew')
        ttk.Label(main_frame, text='Warning Threshold:').grid(row=3, column
            =0, sticky='w')
        ...
        ttk.Button(button_frame, text='Search', command=search).pack(side=
            tk.LEFT, padx=5)
        ttk.Button(button_frame, text='Cancel', command=dialog.destroy).pack(
            side=tk.LEFT, padx=5)
        ...
        ttk.Label(filter_frame, text='Type:').grid(row=0, column=0, sticky=
            'w', padx=5)
        with self.db.session_scope() as session:
        ...
        ttk.Entry(amount_frame, textvariable=amount_var, width=10).pack(side
            =tk.LEFT, padx=2)
        ...
        button_frame = ttk.Frame(main_frame)
        button_frame.pack(fill='x', pady=10)
        ttk.Button(button_frame, text='Apply', command=apply_filters).pack(side
            =tk.LEFT, padx=5)
        ttk.Button(button_frame, text='Cancel', command=dialog.destroy).pack(
            side=tk.LEFT, padx=5)
