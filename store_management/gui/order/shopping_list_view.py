...
class ShoppingListView(ttk.Frame):
    ...
    def show_add_part_dialog(self, parent):
        """Show dialog for adding a new part"""
        # Placeholder for part creation dialog
        messagebox.showinfo("Info", "Add part functionality not implemented yet")
        logger.info("Part creation dialog requested but not implemented")

    def show_add_leather_dialog(self, parent):
        """Show dialog for adding a new leather"""
        # Placeholder for leather creation dialog
        messagebox.showinfo("Info", "Add leather functionality not implemented yet")
        logger.info("Leather creation dialog requested but not implemented")
        ...
        try:
            with self.db.session_scope() as session:
                ...
                if shopping_list:
                    ...
                    logger.info(f"Updated status of shopping list {list_id} to {new_status}")
                    return True
        ...
        try:
            with self.db.session_scope() as session:
                if action_type == 'readd_list':
                    ...
                    # Find items by data since the original IDs are lost
                    for item_data in items_data:
                        # Create a filter to find the matching item
                        filters = [
                            ShoppingListItem.shopping_list_id == list_id,
                            ShoppingListItem.amount == item_data['amount'],
                            ShoppingListItem.price == item_data['price']
                        ]

                        if item_data['part_id']:
                            filters.append(ShoppingListItem.part_id == item_data['part_id'])
                        if item_data['leather_id']:
                            filters.append(ShoppingListItem.leather_id == item_data['leather_id'])

                        # Find and delete the item
                        item = session.query(ShoppingListItem).filter(*filters).first()
                        if item:
                            session.delete(item)
            ...
            logger.error(f"Failed to update shopping list status: {e}")
            return False
        ...
        try:
            with self.db.session_scope() as session:
                ...
                if shopping_list:
                    ...
                    logger.info(f"Updated urgency of shopping list {list_id} to {new_urgency}")
                    return True
            ...
            logger.error(f"Failed to update shopping list urgency: {e}")
            return False
    ...
    def on_list_select(self, event):
        ...
        try:
            with self.db.session_scope() as session:
                ...
                shopping_list = session.query(ShoppingList).join(
            ...
            logger.error(f"Failed to select shopping list: {e}")
        ...
        try:
            with self.db.session_scope() as session:
                ...
                items = session.query(ShoppingListItem).filter(
                    ...
                    ShoppingListItem.part
                ).outerjoin(
                    ShoppingListItem.leather
                ).all()
                ...
                logger.info(f"Loaded {len(items)} items for shopping list {list_id}")

        except SQLAlchemyError as e:
            messagebox.showerror('Error', f'Failed to load list items: {str(e)}')
            logger.error(f"Failed to load list items: {e}")
    ...
    def show_add_item_dialog(self):
        ...
        try:
            with self.db.session_scope() as session:
                ...
                ttk.Spinbox(main_frame, from_=1, to=1000, textvariable=amount_var).grid(row=1, column=1, sticky='ew')
            ...
            logger.error(f"Failed to load items for dialog: {e}")
            dialog.destroy()
            return
            ...
            try:
                ...
                with self.db.session_scope() as session:
                    ...
                    if unique_id.startswith('P'):
                        part = session.query(Part).filter(Part.unique_id == unique_id).first()
                        item_data['part_id'] = part.id
                    else:
                        leather = session.query(Leather).filter(Leather.unique_id == unique_id).first()
                    ...
                    logger.info(f"Added item to shopping list {self.current_list_id}")
