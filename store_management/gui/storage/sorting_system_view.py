# sorting_system_view.py
"""
Sorting System View module for managing sorting system data in a tkinter-based GUI.

This module provides a view for managing sorting system items, including functionalities
like adding, editing, deleting, searching, and filtering items. It utilizes SQLAlchemy ORM
for database operations and includes undo/redo functionality.
"""
...
class SortingSystemView(ttk.Frame):
    """View for managing sorting system items with CRUD operations and undo/redo functionality."""

    @inject(MaterialService)
    def __init__(self, parent: ttk.Frame, session_factory: SessionLocal = SessionLocal):
        """
    ...
    def setup_table(self) -> None:
        ...
        self.columns = ["id", "unique_id", "name", "amount", "warning_threshold", "bin", "notes"]
    ...
    def load_data(self) -> None:
        ...
        try:
            ...
            storages = session.execute(
                select(Storage).options(joinedload(Storage.product)).order_by(Storage.bin)
            ).scalars().all()
    ...
    def show_add_dialog(self) -> None:
        """Show the dialog for adding a new storage item."""
        dialog = AddDialog(
            self,
    ...
    @inject(MaterialService)
    def save_new_item(self, data: Dict) -> None:
        """
        ...
        session = self.session_factory()
        try:
            product = session.execute(select(Product).where(Product.name == data["name"])).scalar_one_or_none()
            if not product:
                product = Product(
    ...
    def delete_selected(self) -> None:
        """Delete the selected storage items."""
