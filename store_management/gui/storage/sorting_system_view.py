...
class SortingSystemView(ttk.Frame):
        ...
        ttk.Button(toolbar, text='Redo', command=self.redo).pack(side=tk.
            RIGHT, padx=2)
        ttk.Button(toolbar, text='Save', command=self.save_table).pack(side
            =tk.RIGHT, padx=2)
        ttk.Button(toolbar, text='Load', command=self.load_table).pack(side
            =tk.RIGHT, padx=2)
        ...
        try:
            if action_type == 'delete':
                ...
                for item_data in data:
                    storage = Storage(id=item_data['id'], product_id=
                        item_data['product_id'], amount=item_data['amount'],
