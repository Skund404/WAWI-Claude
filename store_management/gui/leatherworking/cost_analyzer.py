...
class ProjectCostAnalyzer(BaseView):
    ...
    """
        ...
        type_combo = ttk.Combobox(self.toolbar, textvariable=self.type_var,
            values=['ALL'] + [t.name for t in ProjectType], state=
            'readonly', width=15)
        ...
        try:
            ...
            for widget in labor_frame.winfo_children():
                widget.destroy()
            ttk.Label(labor_frame, text=f'Hours: {labor_hours}').pack(side=
                tk.LEFT)
            ttk.Label(labor_frame, text=f'Rate: ${self.labor_rate}/hr').pack(
                side=tk.LEFT, padx=10)
            ttk.Label(labor_frame, text=f'Total: ${labor_cost:.2f}').pack(side
                =tk.RIGHT)
            ...
            ttk.Label(overhead_frame, text='15% of base cost').pack(side=tk
                .LEFT)
            ttk.Label(overhead_frame, text=f'${overhead_cost:.2f}').pack(side
                =tk.RIGHT)
