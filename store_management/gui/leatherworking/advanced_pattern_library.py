...
class AdvancedPatternLibrary(BaseView):
    ...
    """
        ...
        self.pattern_list_frame.grid(row=0, column=0, sticky='nsew', padx=5,
            pady=5)
        self.setup_pattern_list()
        self.pattern_details_frame = ttk.Frame(self.pattern_frame)
        self.pattern_details_frame.grid(row=0, column=1, sticky='nsew',
            padx=5, pady=5)
        ...
        ttk.Entry(self.details_canvas, textvariable=self.pattern_name_var,
            state='readonly').pack(fill='x')
        ttk.Label(self.details_canvas, text='Type:').pack(anchor='w')
        ttk.Entry(self.details_canvas, textvariable=self.pattern_type_var,
            state='readonly').pack(fill='x')
        ttk.Label(self.details_canvas, text='Description:').pack(anchor='w')
        ttk.Entry(self.details_canvas, textvariable=self.pattern_desc_var,
            state='readonly').pack(fill='x')
        self.image_label = ttk.Label(self.details_canvas)
        self.image_label.pack(pady=10)
        self.materials_tree = ttk.Treeview(self.details_canvas, columns=(
            'material', 'quantity'), show='headings')
        ...
        ttk.Button(self.toolbar, text='Edit Pattern', command=self.edit_pattern
            ).pack(side='left', padx=5)
        ttk.Button(self.toolbar, text='Delete Pattern', command=self.
            delete_pattern).pack(side='left', padx=5)
        ttk.Button(self.toolbar, text='Import Patterns', command=self.
            import_patterns).pack(side='left', padx=5)
        ttk.Button(self.toolbar, text='Export Patterns', command=self.
        ...
        for pattern_id, pattern in self.patterns.items():
            self.pattern_tree.insert('', 'end', pattern_id, values=(pattern
                ['name'], pattern['type']))
        ...
        if not self.current_pattern:
            messagebox.showwarning('Warning',
                'Please select a pattern to edit.')
            return
        dialog = PatternDialog(self, 'Edit Pattern', self.patterns[self.
            current_pattern])
        if dialog.result:
            try:
                updated_pattern = dialog.result
                self.project_service.update_pattern(self.current_pattern,
                    updated_pattern)
        ...
        if file_path:
            try:
                ...
                messagebox.showinfo('Import Successful',
                    f'Imported {len(imported_patterns)} patterns.')
            except Exception as e:
        ...
        if file_path:
            try:
                ...
                messagebox.showinfo('Export Successful',
                    f'Exported {len(self.patterns)} patterns.')
            except Exception as e:
        ...
        self.logger.info(
            f'Ran test suite: {result.testsRun} tests, {len(result.failures)} failures, {len(result.errors)} errors'
            )
...
class PatternDialog(tk.Toplevel):
    """Dialog for adding or editing a pattern."""
        ...
        ttk.Label(self, text='Type:').grid(row=1, column=0, sticky='e',
            padx=5, pady=5)
        ...
        self.materials_tree = ttk.Treeview(self.materials_frame, columns=(
            'material', 'quantity'), show='headings')
        self.materials_tree.heading('material', text='Material')
        self.materials_tree.heading('quantity', text='Quantity')
        self.materials_tree.pack(fill='x', expand=True)
        for material in self.materials:
            self.materials_tree.insert('', 'end', values=(material['name'],
                material['quantity']))
        ...
        if dialog.result:
            ...
            self.materials_tree.insert('', 'end', values=(dialog.result[
    ...
    class MaterialDialog(tk.Toplevel):
        """Dialog for adding a material to a pattern."""
            ...
            self.name_entry = ttk.Entry(self)
            self.name_entry.grid(row=0, column=1, sticky='ew', padx=5, pady=5)
            ttk.Label(self, text='Quantity:').grid(row=1, column=0, sticky=
                'e', padx=5, pady=5)
            self.quantity_entry = ttk.Entry(self)
            self.quantity_entry.grid(row=1, column=1, sticky='ew', padx=5,
                pady=5)
            ttk.Button(self, text='Save', command=self.save).grid(row=2,
                column=0, pady=10)
            ttk.Button(self, text='Cancel', command=self.cancel).grid(row=2,
                column=1, pady=10)
