...
class MaterialUsageTracker(BaseView):
    ...
    def create_toolbar(self) -> None:
        ...
        type_combo = ttk.Combobox(self.toolbar, textvariable=self.type_var,
                                   values=['ALL'] + [t.name for t in MaterialType], state='readonly', width=15)
        ...
        for text, value in [('Week', 'week'), ('Month', 'month'), ('Year', 'year')]:
            ttk.Radiobutton(self.toolbar, text=text, variable=self.range_var, value=value,
                             command=self.update_visualization).pack(side=tk.LEFT, padx=2)
        ttk.Button(self.toolbar, text='Export Data', command=self.export_data).pack(side=tk.RIGHT, padx=5)
    ...
    def create_material_list(self) -> None:
        ...
        self.material_tree = ttk.Treeview(frame, columns=columns, show='headings', height=15)
        headings = {'name': 'Material Name', 'type': 'Type', 'usage': 'Total Usage', 'efficiency': 'Efficiency'}
    ...
    def create_analytics_section(self) -> None:
        ...
        row = 0
        col = 0
        metrics = [('Total Usage', 'total_usage'), ('Average per Project', 'avg_per_project'),
                   ('Efficiency Rate', 'efficiency_rate'), ('Waste Rate', 'waste_rate'),
                   ('Cost per Unit', 'cost_per_unit'), ('Projected Usage', 'projected_usage')]
        ...
        self.trend_text = tk.Text(trend_frame, height=4, wrap=tk.WORD, state=tk.DISABLED)
    ...
    def create_visualization(self) -> None:
        ...
        legend_items = [('Usage', 'blue'), ('Waste', 'red'), ('Efficiency', 'green')]
    ...
    def load_data(self) -> None:
        ...
        try:
            ...
            for material in materials:
                stats = self.material_service.get_material_efficiency_stats(material.id, days=self.get_days_for_range())
                values = (material.name, material.material_type.name,
                          f"{stats['total_usage']:.2f}", f"{stats['efficiency']:.1f}%")
                self.material_tree.insert('', tk.END, values=values, tags=(str(material.id),))
    ...
    def load_material_data(self, material_id: int) -> None:
        ...
        """
        try:
            self.usage_data = self.material_service.get_material_usage_history(material_id, days=self.get_days_for_range())
            self.update_metrics()
    ...
    def calculate_cost_per_unit(self) -> str:
        ...
        try:
            ...
            recent_orders = self.material_service.get_recent_orders(self.current_material_id, days=90)
    ...
    def update_trend_analysis(self) -> None:
        ...
        if not self.usage_data:
            ...
            return
        try:
            current_period = sum(log.quantity_used for log in self.usage_data if (datetime.now() - log.date).days <= self.get_days_for_range() // 2)
            previous_period = sum(log.quantity_used for log in self.usage_data if self.get_days_for_range() >= (datetime.now() - log.date).days > self.get_days_for_range() // 2)
            ...
            if abs(change_pct) < 5:
                ...
                trend_text.append(f"{'Increased' if change_pct > 0 else 'Decreased'} by {abs(change_pct):.1f}%. ")

            current_efficiency = self.calculate_period_efficiency(0, self.get_days_for_range() // 2)
            previous_efficiency = self.calculate_period_efficiency(self.get_days_for_range() // 2, self.get_days_for_range())
            if current_efficiency and previous_efficiency:
                ...
                if abs(eff_change) < 2:
                    ...
                    trend_text.append(f"{'Improved' if eff_change > 0 else 'Declined'} by {abs(eff_change):.1f}%. ")
    ...
    def calculate_period_efficiency(self, start_days: int, end_days: int) -> Optional[float]:
        ...
        """
        try:
            period_logs = [log for log in self.usage_data if end_days >= (datetime.now() - log.date).days > start_days]
            if not period_logs:
                return None
    ...
    def group_data_by_periods(self) -> List[Dict[str, Any]]:
        ...
        """
        periods = []
        period_length = {'week': timedelta(days=7), 'month': timedelta(days=30), 'year': timedelta(days=365)}[self.range_var.get()]
        current_date = datetime.now()
        start_date = current_date - timedelta(days=self.get_days_for_range())
        current_period = start_date
        while current_period <= current_date:
            period_logs = [log for log in self.usage_data if current_period <= log.date < current_period + period_length]
    ...
    def draw_axes(self, margin: int, width: int, height: int) -> None:
        ...
        """
        self.canvas.create_line(margin, height - margin, margin, margin, width=2)
        self.canvas.create_line(margin, height - margin, width - margin, height - margin, width=2)

    @inject(MaterialService)
    def draw_usage_line(self, periods: List[Dict[str, Any]], margin: int, x_scale: float, y_scale: float, height: int) -> None:
        """
    ...
    @inject(MaterialService)
    def draw_efficiency_line(self, periods: List[Dict[str, Any]], margin: int, x_scale: float, y_scale: float, height: int) -> None:
        """
        ...
        for i, data in enumerate(periods):
            ...
            points.extend([x, y])
            if len(points) >= 4:
                self.canvas.create_line(*points, fill='green', width=2, smooth=True, dash=(5, 2))

    @inject(MaterialService)
    def draw_period_labels(self, periods: List[Dict[str, Any]], margin: int, x_scale: float, height: int) -> None:
        """
        ...
        Args:
            ...
            height: Canvas height
        """
        format_str = {'week': '%a', 'month': '%d', 'year': '%b'}[self.range_var.get()]
        for i, data in enumerate(periods):
            if i % 2 == 0:
                x = margin + i * x_scale
                self.canvas.create_text(x, height - margin + 15, text=data['date'].strftime(format_str), anchor=tk.N)
    ...
    def export_data(self) -> None:
        ...
        if not self.current_material_id:
            tk.messagebox.showwarning('Export', 'Please select a material to export data for.')
            return
        try:
            file_path = tk.filedialog.asksaveasfilename(defaultextension='.csv', filetypes=[('CSV files', '*.csv'), ('All files', '*.*')])
            if not file_path:
                return
            self.material_service.export_usage_data(self.current_material_id, file_path, self.get_days_for_range())
