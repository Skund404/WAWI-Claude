...
class MaterialCalculator(tk.Frame):
    ...
    """
        ...
        self.summary_frame = ttk.LabelFrame(self, text=
            'Calculation Summary', padding=(10, 10))
        ...
        add_btn = ttk.Button(self.input_frame, text='Add Material', command
            =self.add_material)
        ...
        self.material_tree_context_menu.add_command(label='Delete', command
        ...
        try:
            filename = simpledialog.askstring('Export Report',
                'Enter filename for report:', initialvalue=
                'material_report.csv')
            if not filename:
                return
