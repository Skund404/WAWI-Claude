...
class CostEstimator(tk.Frame):
    ...
    @inject(MaterialService)
        ...
        category_dropdown = ttk.Combobox(self.input_frame, textvariable=self.category_var, values=cost_categories, state='readonly',
                                         width=27)
        ...
        self.cost_tree = ttk.Treeview(
            self.visualization_frame, columns=columns, show='headings')
        for col in columns:
        ...
        scrollbar = ttk.Scrollbar(
            self.visualization_frame, orient='vertical', command=self.cost_tree.yview)
        ...
        self.cost_tree_context_menu.add_command(
            label='Delete', command=self.delete_selected_cost_item)
        ...
        calculate_btn = ttk.Button(
            btn_frame, text='Recalculate', command=self.recalculate_costs)
        calculate_btn.pack(side='left', expand=True, fill='x', padx=5)
        export_btn = ttk.Button(
            btn_frame, text='Export Report', command=self.export_cost_report)
        ...
        try:
            filename = simpledialog.askstring('Export Report',
                                              'Enter filename for cost report:', initialvalue='cost_report.csv')
            if not filename:
                return
