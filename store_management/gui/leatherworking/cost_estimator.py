...
class CostEstimator(tk.Frame):
    ...
    """
        ...
        self.input_frame = ttk.LabelFrame(self, text='Cost Input', padding=
            (10, 10))
        self.input_frame.grid(row=0, column=0, sticky='nsew', padx=5, pady=5)
        self.visualization_frame = ttk.LabelFrame(self, text=
            'Cost Breakdown', padding=(10, 10))
        self.visualization_frame.grid(row=0, column=1, sticky='nsew', padx=
            5, pady=5)
        self.summary_frame = ttk.LabelFrame(self, text='Cost Summary',
            padding=(10, 10))
        self.summary_frame.grid(row=1, column=0, columnspan=2, sticky=
            'nsew', padx=5, pady=5)
        ...
        """
        ttk.Label(self.input_frame, text='Cost Category:').grid(row=0,
            column=0, sticky='w', pady=5)
        self.category_var = tk.StringVar()
        cost_categories = ['Leather', 'Hardware', 'Thread', 'Tools',
            'Adhesive', 'Lining', 'Labor', 'Miscellaneous']
        category_dropdown = ttk.Combobox(self.input_frame, textvariable=
            self.category_var, values=cost_categories, state='readonly',
            width=27)
        category_dropdown.grid(row=0, column=1, sticky='ew', pady=5)
        ttk.Label(self.input_frame, text='Item Name:').grid(row=1, column=0,
            sticky='w', pady=5)
        self.item_name_var = tk.StringVar()
        item_name_entry = ttk.Entry(self.input_frame, textvariable=self.
            item_name_var, width=30)
        item_name_entry.grid(row=1, column=1, sticky='ew', pady=5)
        ttk.Label(self.input_frame, text='Quantity:').grid(row=2, column=0,
            sticky='w', pady=5)
        self.quantity_var = tk.StringVar(value='1')
        quantity_entry = ttk.Entry(self.input_frame, textvariable=self.
            quantity_var, width=30)
        quantity_entry.grid(row=2, column=1, sticky='ew', pady=5)
        ttk.Label(self.input_frame, text='Unit Price ($):').grid(row=3,
            column=0, sticky='w', pady=5)
        self.unit_price_var = tk.StringVar()
        unit_price_entry = ttk.Entry(self.input_frame, textvariable=self.
            unit_price_var, width=30)
        unit_price_entry.grid(row=3, column=1, sticky='ew', pady=5)
        ttk.Label(self.input_frame, text='Hours (Labor):').grid(row=4,
            column=0, sticky='w', pady=5)
        self.hours_var = tk.StringVar(value='0')
        hours_entry = ttk.Entry(self.input_frame, textvariable=self.
            hours_var, width=30)
        hours_entry.grid(row=4, column=1, sticky='ew', pady=5)
        ttk.Label(self.input_frame, text='Hourly Rate ($):').grid(row=5,
            column=0, sticky='w', pady=5)
        self.hourly_rate_var = tk.StringVar(value='25')
        hourly_rate_entry = ttk.Entry(self.input_frame, textvariable=self.
            hourly_rate_var, width=30)
        hourly_rate_entry.grid(row=5, column=1, sticky='ew', pady=5)
        add_btn = ttk.Button(self.input_frame, text='Add Cost Item',
        ...
        self.cost_tree = ttk.Treeview(self.visualization_frame, columns=
        ...
        scrollbar = ttk.Scrollbar(self.visualization_frame, orient=
            'vertical', command=self.cost_tree.yview)
        self.cost_tree.configure(yscroll=scrollbar.set)
        scrollbar.pack(side='right', fill='y')
        self.cost_tree_context_menu = tk.Menu(self, tearoff=0)
        self.cost_tree_context_menu.add_command(label='Delete', command=
            self.delete_selected_cost_item)
        ...
        summary_metrics = [('Total Materials Cost', 'materials_cost'), (
            'Labor Cost', 'labor_cost'), ('Total Project Cost',
            'total_cost'), ('Estimated Profit Margin (%)', 'profit_margin')]
        ...
        calculate_btn = ttk.Button(btn_frame, text='Recalculate', command=
            self.recalculate_costs)
        calculate_btn.pack(side='left', expand=True, fill='x', padx=5)
        export_btn = ttk.Button(btn_frame, text='Export Report', command=
        ...
        try:
            ...
            cost_item = {'category': category, 'item_name': item_name,
                'quantity': quantity, 'unit_price': unit_price,
                'total_cost': total_cost}
            self.cost_items.append(cost_item)
            self.cost_tree.insert('', 'end', values=(category, item_name,
                f'{quantity}', f'${unit_price:.2f}', f'${total_cost:.2f}'))
            self.recalculate_costs()
        ...
        """
        materials_cost = sum(item['total_cost'] for item in self.cost_items if
            item['category'] != 'Labor')
        self.materials_cost_label.config(text=f'${materials_cost:.2f}')
        labor_cost = sum(item['total_cost'] for item in self.cost_items if 
            item['category'] == 'Labor')
        ...
        profit_margin = (estimated_profit / estimated_selling_price * 100 if
            estimated_selling_price > 0 else 0)
        ...
        for item in self.cost_items:
            categories[item['category']] = categories.get(item['category'], 0
                ) + item['total_cost']
        if categories:
            self.ax.pie(list(categories.values()), labels=list(categories.
                keys()), autopct='%1.1f%%')
        else:
        ...
        if not selected_item:
            messagebox.showwarning('Selection',
                'Please select a cost item to delete.')
            return
        ...
        try:
            filename = simpledialog.askstring('Export Report',
                'Enter filename for cost report:', initialvalue=
                'cost_report.csv')
            if not filename:
                return
            with open(filename, 'w') as f:
                ...
                for item in self.cost_items:
                    f.write(
                        f"""{item['category']},{item['item_name']},{item['quantity']},{item['unit_price']},{item['total_cost']}
"""
                        )
            f.write('\nSummary,,,\n')
            f.write(
                f"Materials Cost,,,{self.materials_cost_label.cget('text')}\n")
            f.write(f"Labor Cost,,,{self.labor_cost_label.cget('text')}\n")
            f.write(
                f"Total Project Cost,,,{self.total_cost_label.cget('text')}\n")
            f.write(
                f"Estimated Profit Margin,,,{self.profit_margin_label.cget('text')}\n"
                )
            messagebox.showinfo('Export Successful',
                f'Cost report exported to {filename}')
        ...
        """
        initial_costs = [{'category': 'Leather', 'item_name':
            'Full Grain Leather', 'quantity': 2, 'unit_price': 50.0,
            'total_cost': 100.0}, {'category': 'Hardware', 'item_name':
            'Metal Buckles', 'quantity': 4, 'unit_price': 5.5, 'total_cost':
            22.0}, {'category': 'Labor', 'item_name': 'Design and Cutting',
            'quantity': 3, 'unit_price': 25.0, 'total_cost': 75.0}]
        for cost_item in initial_costs:
            self.cost_items.append(cost_item)
            self.cost_tree.insert('', 'end', values=(cost_item['category'],
                cost_item['item_name'], f"{cost_item['quantity']}",
