...
class TimelineViewer(tk.Frame):
    ...
    def _create_project_input(self):
        ...
        project_name_entry = ttk.Entry(self.input_frame, textvariable=self.project_name_var, width=30)
        ...
        start_date_entry = DateEntry(self.input_frame, textvariable=self.start_date_var, width=27, date_pattern='y-mm-dd')
        start_date_entry.grid(row=2, column=1, sticky='ew', pady=5)
        ttk.Label(self.input_frame, text='Est. Duration (Days):').grid(
            row=3, column=0, sticky='w', pady=5)
        self.duration_var = tk.StringVar()
        duration_entry = ttk.Entry(self.input_frame, textvariable=self.duration_var, width=30)
        ...
        status_dropdown = ttk.Combobox(self.input_frame, textvariable=self.status_var, values=status_options, state='readonly', width=27)
    ...
    def add_project(self):
        ...
        try:
            ...
            project = {'name': project_name, 'type': project_type,
                       'start_date': start_date, 'duration': duration, 'status': status}
            self.projects.append(project)
            self.project_tree.insert('', 'end', values=(project['name'],
                                                         project['type'], project['start_date'].strftime('%Y-%m-%d'),
    ...
    def update_timeline_chart(self):
        ...
        for idx, project in enumerate(sorted_projects):
            ...
            self.ax.barh(project['name'], project['duration'], left=project['start_date'].toordinal(),
                          color=status_colors.get(project['status'], 'lightgray'), alpha=0.7)
        plt.tight_layout()
        self.canvas.draw()
