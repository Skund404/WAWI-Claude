...
class TimelineViewer(tk.Frame):
    ...
    """
        ...
        self.input_frame = ttk.LabelFrame(self, text=
            'Project Timeline Input', padding=(10, 10))
        ...
        self.list_frame = ttk.LabelFrame(self, text='Project List', padding
            =(10, 10))
        ...
        project_type_dropdown = ttk.Combobox(self.input_frame, textvariable
            =self.project_type_var, values=project_types, state='readonly',
            width=27)
        project_type_dropdown.grid(row=1, column=1, sticky='ew', pady=5)
        ttk.Label(self.input_frame, text='Start Date:').grid(row=2, column=
            0, sticky='w', pady=5)
        ...
        ttk.Label(self.input_frame, text='Est. Duration (Days):').grid(row=
            3, column=0, sticky='w', pady=5)
        ...
        add_btn = ttk.Button(self.input_frame, text='Add Project', command=
            self.add_project)
        ...
        self.project_tree_context_menu.add_command(label='Delete', command=
        ...
        for idx, project in enumerate(sorted_projects):
            end_date = project['start_date'] + datetime.timedelta(days=
                project['duration'])
