...
class OrderService(Service, IOrderService):
    ...
    def get_orders_by_date_range(self, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        ...
        """
        try:
            orders = self._session.query(Order).filter(Order.order_date.between(start_date, end_date)).all()
            return [order.to_dict(include_items=True) for order in orders]
        except SQLAlchemyError as e:
    ...
    def get_supplier_orders(self, supplier_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            orders = self._session.query(Order).filter(Order.supplier_id == supplier_id).all()
            return [order.to_dict(include_items=True) for order in orders]
    ...
    def generate_order_report(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            for status in OrderStatus:
                report['orders_by_status'][status.value] = sum(1 for order in orders if order.status == status)

            # Calculate totals by supplier
            supplier_totals = {}
            for order in orders:
                supplier_totals[order.supplier_id] = supplier_totals.get(order.supplier_id, 0) + order.total_amount

            # Get top 5 suppliers by order value
            report['top_suppliers'] = dict(sorted(supplier_totals.items(), key=lambda x: x[1], reverse=True)[:5])
